...
Thus, each point $(x,y)$ on the hyperbola in the plot on the left maps to two points on the line in the plot on the right: $(-1, 2x\sqrt{1+x^2})$, and $(-1, 2x\sqrt{1+x^2})$. Thus, the line in the plot on the right has arrows pointing in both directions.
...
(We are indexing from 0 in this question)

\begin{proof}
    let $A = [z_{jk}]$ be an $m \times n$ matrix.
    Then, the $jk^{\text{th}}$ entry in $A^HA$, denoted $a_{jk}$, is $$\sum\limits_{l=0}^{n-1} \overline{z_{jl}}z_{kl}.$$
    Further, the $kj^{\text{th}}$ entry in $A^HA$, denoted $a_{kj}$, is
    \begin{align*}
        \sum\limits_{l=0}^{n-1} \overline{z_{kl}}z_{jl} &= \sum\limits_{l=0}^{n-1} \overline{z_{kl}\overline{z_{jl}}} \\
        &= \overline{\sum\limits_{l=0}^{n-1} z_{kl}\overline{z_{jl}}} \\
        &= \overline{\sum\limits_{l=0}^{n-1} \overline{z_{jl}}z_{kl}} \\
        &= \overline{a_{jk}}.
    \end{align*}
